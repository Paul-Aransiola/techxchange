...
import { reviewInputType } from "../../controllers/types/controller";
import { reviewModel, productModel, userModel } from "../../database/models";
import { mongooseTransaction } from "../../database/utils";


const getAllReviews = async (productId: string): Promise<ReviewModelInterface[]> => {
  return await reviewModel.find({ product: productId }).populate('product').populate("user");
};
